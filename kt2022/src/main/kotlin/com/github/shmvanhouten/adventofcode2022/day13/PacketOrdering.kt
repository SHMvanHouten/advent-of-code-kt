...
fun findIndicesOfCorrectPacketPairs(input: String): List<Int> {
    return input.blocks().withIndex().filter {
        sortedCorrectly(it.value.lines())
    }.map { it.index + 1 }
}
...
private fun sortedCorrectly(pairs: List<String>): Boolean {
    return sortedCorrectly(pairs[0], pairs[1])
}

fun sortedCorrectly(l: String, r: String): Boolean {
