...
fun findCheats(input: String, cheatLength: Int = 2): Sequence<CheatingResult> {
    ...
    return possibleCheatsAlongPath2(grid, quickestPath, cheatLength)
        .asSequence()
        .mapNotNull { cheat ->
...
fun possibleCheatsAlongPath2(grid: Grid<Char>, quickestPath: Path, cheatLength: Int): List<Cheat> {
...
fun possibleCheatsAlongPath(grid: Grid<Char>, quickestPath: Path, cheatLength: Int): List<Cheat2> {
    val steps = quickestPath.steps
    return steps.subList(0, steps.lastIndex).withIndex().asSequence()
        .map { (index, step) -> step to quickestPath.steps.subList(0, index + 1) }
        .flatMap { (step, pathSoFar) -> grid.listAllOpenSpotsWithIn(step, cheatLength, pathSoFar.toSet())
...
private const val cheatChars = "0123456789abcdefghijklmnop"
...
data class Path(
    ...
    fun print(grid: Grid<Char>): String {
        ...
        steps.forEach { mutableGrid[it] = '0' }
...
fun Grid<Char>.listAllOpenSpotsWithIn(start: Coordinate, length: Int, mustNotEndIn: Set<Coordinate>): Set<Coordinate> {
    val startY = maxOf(start.y - length, 1)
    val endY = minOf(start.y + length, this.height - 2)
    return (startY..endY).flatMap { y ->
        val availableSteps = length - abs(start.y - y)
        val startX = maxOf(start.x - availableSteps, 1)
        val endX = minOf(start.x + availableSteps, this.width - 2)
