...
class RangeOverlapTest {
    ...
    fun `1-3 does not overlap with 4-5`() {
        val (overlapping, notOverlapping) = (1L..3).leftPartitionOverlapping(4L..5)
        expect {
    ...
    fun `5-6 does not overlap with 3-4`() {
        val (overlapping, notOverlapping) = (5L..6).leftPartitionOverlapping(3L..4)
        expect {
            that(overlapping).isEmpty()
            that(notOverlapping)
                .hasSize(1)
    ...
    fun `1-4 overlaps with 4-5 on 4`() {
        // 1--4..
        // ...45.
        val (overlapping, notOverlapping) = (1L..4).leftPartitionOverlapping(4L..5)
        expect {
    ...
    fun `5-6 overlaps with 3-5 on 5`() {
        // ....56...
        // ..3-5....
        val (overlapping, notOverlapping) = (5L..6).leftPartitionOverlapping(3L..5)
        expect {
            that(overlapping).hasSize(1)
    ...
    fun `5-9 overlaps with 3-6 on 5-6`() {
        ...
        // ....56...
        // ......7-9
        val (overlapping, notOverlapping) = (5L..9).leftPartitionOverlapping(3L..6)
        expect {
            that(overlapping).hasSize(1)
                .first().isEqualTo(5L..6)
            that(notOverlapping).hasSize(1)
    ...
    fun `1-9 completely contains 4-5`() {
        val (overlapping, notOverlapping) = (1L..9).leftPartitionOverlapping(4L..5)
        expect {
            that(overlapping).hasSize(1)
    ...
    fun `1--9 overlaps with 4-4 and 5-7 fully`() {
        val (overlapping, notOverlapping) = (1L..9)
            .leftPartitionOverlapping(listOf(4L..4, 5L..7))
    ...
    fun `3--7 overlaps with 1-4 partially and 5-5 fully and 7-8 partially`() {
        val (overlapping, notOverlapping) = (3L..7)
            .leftPartitionOverlapping(listOf(1L..4, 5L..5, 7L..8))
    ...
    fun `reverse list example`() {
        val (overlapping, notOverlapping) = (79L..92)
            .leftPartitionOverlapping(listOf(98L..99, 50L..97))
